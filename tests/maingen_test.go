...
// Test 4 — even with routes/routes.go present, an existing web/server.go is the
// escape hatch: it is not overwritten and no main is generated. (The decision
// logic itself is covered white-box in the root package; this asserts the
// observable outcome without compiling anything.)
func TestEscapeHatch_UserMainNotOverwritten(t *testing.T) {
	...
	// GenerateMain still writes only the generated artifact, never the user's
	// file; the strategy layer is what declines to call it (see the root-package
	// TestServerMainSelection). What matters here: the user's file is inert to
	// generation.
	if _, err := server.GenerateMain(root, "example.com/app", server.MainConfig{Port: "8080", PublicDir: "web/public"}); err != nil {
		t.Fatalf("GenerateMain: %v", err)
	}

	got, _ := os.ReadFile(userFile)
